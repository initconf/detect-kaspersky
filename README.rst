...
-----------------------
:: 
    a) check for resp_mime_types in the HTTP GET requests to see if its a kaspersky client
   

    if (watched_resp_mime_types in rec$resp_mime_types[mtypes]) 
        NOTICE   

    b) Use a built in list of  IPs + URLs to identify new Kaspersky IPs and URLs 
